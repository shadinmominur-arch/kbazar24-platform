...
- **Remaining items reassigned to Codex 2026-06-10** (owner instruction): C10 remainder (marquee + cart-preview `data-nosnippet` in `Header.tsx`, ~lines 664-700 and 807-844), D13 (ingredient/concern internal links on PDP — new component), D14 (numbered pagination on /shop and /category — new shared component). See X4 below. B4 (hero raw `<img>`→`next/image`) explicitly deferred as separate LCP-sensitive task, not assigned.

---

## 🟡 CODEX — Next tasks

### X4 — 14-item audit remainder (handed off from Claude C5, 2026-06-10)
- **C10 remainder**: add `data-nosnippet` to the homepage marquee and cart-preview duplicate-text blocks in `Header.tsx` (~lines 664-700 and 807-844 as of `ade571e`) — same pattern as `ProductCard.tsx:195` and `HeroCarousel.tsx` mobile copy (commit `592b1eb`).
- **D13**: add ingredient/concern internal links on PDP (new component) — link product ingredient/concern terms to `/ingredients/*` and `/concerns/*` pages, matching the M6 internal-link pattern already used elsewhere.
- **D14**: numbered pagination (new shared component) for `/shop` and `/category/[slug]` — pages already self-canonicalize for `?page=N` (see `project_collection_pagination_seo_20260608`); this adds the visible page-number UI.
- One commit per item, build/lint/typecheck on Local, Local→VPS deploy sequence, smoke test before push. B4 (hero `<img>`→`next/image`) stays deferred/unassigned.
